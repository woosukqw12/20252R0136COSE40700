...
def calculate_robust_f1(true_seqs, pred_seqs, mode="strict"):
    ...
    (1토큰 엔티티 버그 수정 및 중복 매칭 방지 로직 추가)
    ...
    for true_seq, pred_seq in zip(true_seqs, pred_seqs):
        ...
        # seqeval의 End Index는 Inclusive(포함)입니다.
        ...
        for p_ent in pred_ents:
            ...
            for t_idx, t_ent in enumerate(true_ents):
                ...
                # [교집합 계산] (Inclusive Index 기준 보정)
                intersect_start = max(p_start, t_start)
                intersect_end = min(p_end, t_end)
                
                # 끝점이 시작점보다 작으면 겹치지 않는 것
                if intersect_start > intersect_end:
                    overlap_len = 0
                else:
                    overlap_len = intersect_end - intersect_start + 1
...
def compute_advanced_metrics_crf(p):
    ...
    strict_f1 = calculate_robust_f1(true_seqs, pred_seqs, mode="strict")
    overlap_f1 = calculate_robust_f1(true_seqs, pred_seqs, mode="overlap")
    intermediate_f1 = calculate_robust_f1(true_seqs, pred_seqs, mode="intermediate")
